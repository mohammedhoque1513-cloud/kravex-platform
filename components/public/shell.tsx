...
export function PublicNav() {
  ...
  return (
    <>
      <header className="fixed inset-x-0 top-0 z-50 border-b border-kravex-gold/50 bg-black/95 shadow-[0_10px_45px_rgba(0,0,0,.45)] backdrop-blur">
        <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:h-20 sm:px-5">
          ...
          <div className="hidden items-center gap-6 lg:flex">
            {links.map(([label, href]) => <Link key={href} href={href} className="text-xs font-bold uppercase tracking-[.14em] text-kravex-secondary hover:text-kravex-gold">{label}</Link>)}
          </div>
          <div className="hidden items-center gap-5 lg:flex">
            <GoldButton href="/contact">Book a Call</GoldButton>
            <Link href="/login" className="text-xs font-bold uppercase tracking-[.14em] text-kravex-gold">Login</Link>
          </div>
          <button className="rounded-md border border-kravex-border bg-kravex-card p-2 text-kravex-gold lg:hidden" onClick={() => setOpen(true)} aria-label="Open menu">
            <Menu size={22} />
          </button>
        </nav>
      </header>
...
export function PublicFooter() {
  return (
    <footer className="border-t border-kravex-gold/30 bg-black px-4 py-12 sm:px-6 sm:py-14">
      <div className="mx-auto grid max-w-7xl gap-9 sm:grid-cols-2 lg:grid-cols-4">
        <div>
          ...
          <p className="mt-4 text-xs font-bold uppercase tracking-[.16em] text-kravex-gold">LinkedIn / X / Email</p>
        </div>
        <div>
          <h3 className="text-xs font-bold uppercase tracking-[.18em] text-kravex-gold">Navigation</h3>
          <div className="mt-4 grid gap-2 text-sm text-kravex-secondary">{links.map(([label, href]) => <Link key={href} href={href}>{label}</Link>)}</div>
        </div>
        <div>
          <h3 className="text-xs font-bold uppercase tracking-[.18em] text-kravex-gold">Construction Niches</h3>
        ...
        <div>
          <h3 className="text-xs font-bold uppercase tracking-[.18em] text-kravex-gold">Contact</h3>
