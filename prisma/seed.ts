...
async function main() {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const patel = await tx.client.create({ data: { businessName: "Patel Dental", contactName: "Dr. Ravi Patel", email: "patel@pateldental.co.uk", phone: "0121 555 0191", niche: "Cosmetic Dentistry", city: "Birmingham", retainerAmount: 150000, leadTarget: 20, contractStart: new Date("2026-01-01"), status: ClientStatus.ACTIVE, users: { create: { name: "Dr. Ravi Patel", email: "patel@pateldental.co.uk", passwordHash: clientPasswordHash, role: Role.CLIENT } } } });
    ...
    for (const p of [{ businessName:"Harborne Aesthetics", contactName:"Emily Clark", niche:"Aesthetic Clinics", city:"Birmingham", stage:ProspectStage.NEW_LEAD, estimatedValue:180000, type:ActivityType.NOTE, note:"Website enquiry received." },{ businessName:"North Leeds Solicitors", contactName:"Sarah Iqbal", niche:"Legal & Solicitors", city:"Leeds", stage:ProspectStage.CALL_BOOKED, estimatedValue:220000, type:ActivityType.CALL, note:"Discovery call booked." },{ businessName:"Cotswold Mortgage Co", contactName:"James Price", niche:"Mortgage Brokers", city:"Cheltenham", stage:ProspectStage.PROPOSAL_SENT, estimatedValue:300000, type:ActivityType.PROPOSAL, note:"Growth proposal sent." }]) {
      const { type, note, ...prospect } = p;
      await tx.prospect.create({ data: { ...prospect, activities: { create: { type, note } } } });
    }
    const c1 = await tx.campaign.create({ data: { clientId: patel.id, name: "Dental Implant Enquiries", niche: "Cosmetic Dentistry", platform: Platform.GOOGLE_ADS, status: CampaignStatus.ACTIVE, callsBooked: 12, leadsGenerated: 5, monthlyBudget: 180000, amountSpent: 76000 } });
    ...
    await tx.payment.create({ data: { invoiceId: i1.id, clientId: patel.id, amount: 180000, paymentDate: new Date("2026-05-04"), paymentMethod: PaymentMethod.BANK_TRANSFER, reference: "PATEL-MAY-2026" } });
