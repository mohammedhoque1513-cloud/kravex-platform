...
export async function GET() {
  ...
  const secret = account.twoFactorSecret ? decryptSecret(account.twoFactorSecret) : generateSecret();
...
export async function POST(req: NextRequest) {
  ...
  if (!code || !verify({ token: String(code), secret: decryptSecret(account.twoFactorSecret) })) return json({ error: "Enter a valid 2FA code." }, 400);
